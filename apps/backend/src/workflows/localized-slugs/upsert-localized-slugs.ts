...
import {
  assertValidSlug,
  InvalidSlugError,
} from "../../modules/localized-slugs/utils/slug";
...
export async function updateLocalizedSlugsStepInvoke(
  ...
  return new StepResponse(
    updated as LocalizedSlugOutput[],
...
export const upsertLocalizedSlugsWorkflow = createWorkflow(
  ...
  (input: UpsertLocalizedSlugsInput) => {
    ...
    const desiredForConflictCheck = transform(
      { normalized },
      ({ normalized }) =>
    ...
    deleteLocalizedSlugsStep(
      transform({ diff }, ({ diff }) => diff.toDeleteIds),
    );
